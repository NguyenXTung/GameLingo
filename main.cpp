...
#include "modegame.h"
using namespace std;
int ROW = 4; //so hang (so lan doan) = so cot (do dai tu can doan)
...
void game(SDL_Window* window, SDL_Renderer* renderer){
    ...
    while (play) {
        if ( SDL_WaitEvent(&e) == 0) SDL_Delay(100);
                ...
                //input word
                if(e.key.keysym.sym == SDLK_ESCAPE) play = false;
